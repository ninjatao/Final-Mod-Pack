...
CHANGELOG:
4.2: 2 new Rice Farm maps and enabled huge maps in the script. Minor fixes.
...
3.0: New soldier nationalities: Afghani, Algerian, Australian, Austrian, Azeri, Belgian, Chilean, Colombian, Ecuadorian, Egyptian, Iraqi, Libyan, Lithuanian, Moroccan, New Zealander, Pakistani, Peruvian, Tunisian, Turkmen, Venezuelan. Expanded Cuban names. New Junkfarm/Native Urban maps (by Dioxine). New farm map (by Dioxine). New jungle maps and additions (by Dioxine). New 30x30 maps for several terrains. Renamed Plasma Blaster to Plasma Destroyer for clarity. New Ufopedia picture for the Skymarshall (by Nord). Better lift animation (by Brain_322). Better jungle trees (by Dioxine). Fixed Avenger/Dropship cockpit (by Buscher). Fixed outdated Postindustrial tilesets. Minor fixes.
2.9: New soldier nationalities: Armenian, Belarusian, Croatian, Cuban, Estonian, Faroese, Icelandic, Indonesian, Iranian, Kazakh, Kyrgyz, Latvian, Mexican, Mongolian, Saudi Arabian, Senegalese, South African, Swiss, Tajik, Thai, Ugandan, Uzbek, Vietnamese. Some name files expanded. 4 new Jungle maps (collab with Dioxine). 1 new Postindustrial map (by Dioxine). Battleship walls are less transparent. Improved graphics for some ranked aliens (by The Martian).
...
2.2: Changed weights on terrain types (mountainous terrain is picked less often). New terrain: cacti-less desert (for outside of the Americas). New jungle map blocks. Hybrid convoys are now fixed missions (not UFOs). Repairing Stormtrooper armour no longer requires Alien Electronics. Fixed point value of the Scout Drone. Fixed Skymarshall map (by New Civilian). Minor fixes to maps.
2.1: Fixed a crash on Cerebreal attack. Fixed broken language files.
2.0b: Fixed recovery of X-Com armours.
2.0: Converted all GIFs to PNGs. Fixed (most of) broken palettes. Updated to the newest Commendations mod (v3.0). New shotgun handob. Translation updates.
